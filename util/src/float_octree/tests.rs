...
#[test]
fn test_iter_basic() {
    let tree: Octree<i64> = Octree::with_scale(0);
    assert_eq!(values_vec(&tree), Vec::<i64>::new());
    tree.assert_scale_invariant();

    let tree: Octree<i64> = Octree::with_scale(50);
    assert_eq!(values_vec(&tree), Vec::<i64>::new());
    tree.assert_scale_invariant();
...
fn test_dense_iter_mut() {
    let mut tree: Octree<i64> = Octree::with_scale(5);
...
#[test]
fn test_grow() {
    let mut tree: Octree<i64> = Octree::with_scale(7);
    tree.insert(Point3::new(34, 2, 17), 6);
    tree.insert(Point3::new(2, 3, 4), 5);
    tree.insert(Point3::new(34, 3, 16), 2);
    tree.insert(Point3::new(1, 1, 1), 7);
    ...
    tree.grow();
    assert_eq!(tree.scale(), 8);
    check_points(&tree);
    tree.assert_scale_invariant();
    println!("{:?}", tree.iter().collect::<Vec<_>>());

    tree.grow();
    assert_eq!(tree.scale(), 9);
    check_points(&tree);
    tree.assert_scale_invariant();

    tree.grow();
    assert_eq!(tree.scale(), 10);
    check_points(&tree);
    tree.assert_scale_invariant();

    tree.grow();
    assert_eq!(tree.scale(), 11);
    check_points(&tree);
    tree.assert_scale_invariant();

    tree.grow();
    assert_eq!(tree.scale(), 12);
    check_points(&tree);
    tree.assert_scale_invariant();
}
...
#[test]
fn test_cast_ray() {
    let mut tree: Octree<i64> = Octree::with_scale(4);
